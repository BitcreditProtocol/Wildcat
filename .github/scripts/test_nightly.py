...
class NightlyTests(unittest.TestCase):
    def test_exact_source_guard_rejects_moved_master_and_invalid_inputs(self):
        step = next(s for s in WORKFLOW["jobs"]["changes"]["steps"]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                with patch.dict(os.environ, env, clear=True), patch("subprocess.check_output") as inspect:
                    inspect.side_effect = [json.dumps({"digest": "sha256:" + d * 64}) for d in "ab"]
                    ...
                    self.assertEqual(result["references"], {
                        "gar": "registry.example/project/core@sha256:" + "a" * 64,
    ...
    def test_prs_cannot_publish_and_dispatch_cannot_skip_matrix(self):
        ...
        self.assertEqual(jobs["changes"]["if"], "github.event_name != 'pull_request'")
        self.assertEqual(jobs["changes"]["needs"], "test")
