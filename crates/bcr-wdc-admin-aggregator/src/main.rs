...
struct MainConfig {
    ...
    appcfg: bcr_wdc_admin_aggregator::AppConfig,
    log_level: String,
}
...
async fn main() {
    ...
    let maincfg: MainConfig = settings
        .try_deserialize()
        .expect("Failed to parse admin aggregator config");
    ...
    let app = bcr_wdc_admin_aggregator::AppController::new(maincfg.appcfg).await;
    let router = bcr_wdc_admin_aggregator::routes(app);
