...
mod tests {
    ...
    fn test_factory_generate() {
        ...
        let (info1, _) = factory._generate(unit1.clone(), now, Some(expire1), 100);
        assert_eq!(info1.unit, unit1);
        assert_eq!(info1.final_expiry, Some(expire1.timestamp() as u64));
        // different unit
        let unit2 = cashu::CurrencyUnit::Eur;
        let (info2, _) = factory._generate(unit2.clone(), now, Some(expire1), 0);
        ...
        let (info3, _) = factory._generate(unit1.clone(), now, Some(expire3), 0);
        ...
        let (info4, _) = factory._generate(unit1.clone(), now4, Some(expire1), 0);
